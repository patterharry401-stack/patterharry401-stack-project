#!/bin/bash
clear
echo "=============================="
echo "    HARRY-MAX OSINT SEARCH    "
echo "=============================="
echo "Master Harry, please enter a Username or Email."
read -p "Target: " target

if [[ $target == *"@"* ]]; then
    echo "Searching Email: $target..."
    holehe $target | grep "\[+\]"
else
    echo "Searching Username: $target..."
    # Sherlock ရဲ့ ပင်မ Folder ထဲကို ဝင်မယ်
    cd ~/patterharry401-stack-project/sherlock
    # Folder နာမည်ကို Python Module အနေနဲ့ လှမ်းခေါ်မယ် (ဒါမှ သူ မငြင်းတော့တာ)
    python3 -m sherlock_project $target --timeout 1 --print-found
fi

echo "------------------------------"
echo "Search Completed, Master Harry."
